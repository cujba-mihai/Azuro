...
import { BuildingsIcon, GarageIcon, GrayCarIcon, GreenCar, InsuranceIcon, MoneyBagIcon, TravelersIcon, ZenIcon } from 'assets/icons';
import { Card, Text } from 'react-native-paper';
import { FlatList } from 'react-native-gesture-handler';
import { StyledSelectInsuranceCard, StyledSelectInsuranceContainer, StyledSelectInsuranceText } from '../styles';
...
export const SelectInsuranceCard = () => {
    const renderItem = ({ item, index }: { item: typeof INSURANCE_OPTIONS[0], index: number }) => {
        const numberOfRows = Math.ceil(INSURANCE_OPTIONS.length / 2);

        const currentRow = Math.floor(index / 2);

        return (
            <StyledSelectInsuranceContainer href="/select-insurance/modal"
                style={{
                    flex: 1,
